...
> decisions; `docs/scoring.md` records the scoring model (locked and proposed).
...
| Phase 2C — Location scoring      | **RESERVED / NOT IMPLEMENTED** (design-only)  |
| Phase 2C — Workplace scoring     | **RESERVED / NOT IMPLEMENTED** (design-only)  |
...
│   └── scoring.md            # Scoring model (locked + proposed weights)
...
2. **Do NOT implement** composite / location / workplace scoring, or any
   application automation, until the architecture and policies are explicitly approved.
...
## Dataset (20 canonical records, verified 2026-08-11)
...
- All four score files join 1:1 with `job_records.json` on `job_id`.

## Key contacts

- Join key for all score files: **`job_id`**.
- Composite weights are **documented hints only, NOT implemented**:
  Salary 0.15 + Experience 0.20 + Skills 0.30 + Role 0.15 = **0.80** (sums to
  0.80 — see `docs/scoring.md` before use; Location/Workplace not yet locked).
